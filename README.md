...
FamilyFlow 是一个可运行、可测试、可容器化部署的全栈 MVP。用户验证卡密并提交接收邮箱后，后端会在数据库事务中选择仍有余量的母号、预占席位、创建邀请任务，再通过 Redis 与 BullMQ 异步处理；前端持续展示脱敏后的任务状态。

> [!IMPORTANT]
> 本项目是第三方学习与工程实践项目，与 Google 无隶属、授权或官方合作关系。默认使用 `MockInvitationExecutor` 演示完整流程，不会登录或控制 Google 账号，也不会真正发送家庭组邀请。
